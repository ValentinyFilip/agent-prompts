...
3. **Execution & Verification**: I execute the requested code change. Even in Vibe Mode, I must perform pragmatic testing:
   - For backend/core logic, I run the test suite via the terminal or the active semantic tool runner (as governed by the `# Semantic Code Intelligence Protocol` chapter in `.clinerules/local-tools.md`) to verify that my changes are correct before marking the task complete.
