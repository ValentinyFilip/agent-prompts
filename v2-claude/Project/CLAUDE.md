...
- **Dual-File Compression Protocol**: Do NOT delete `*.original.md` files—they are required for human developers and Git diff reviews. Immediately generate the Caveman-compressed counterpart (`*.md`) with `/steering-manager compress <path>`. The steering manager performs the compression itself. Do not call an external compression skill.
...
6. Announce Spec Mode or Vibe Mode. Use the exact sentence in `~/.claude/rules/sdd-tdd-engine.md`.

Do not write code, create a plan document, or edit a file before step 6 is complete.
...
Use the semantic tools in that rule before you open files recursively.
...
- **`/steering-manager` ships with this rule set.** Use it for every create, update, audit, and compress operation on `.steering/`. It is not an external dependency.
...
5. **Steering safety**: Never edit a file in `.steering/` directly. Use `/steering-manager`.
